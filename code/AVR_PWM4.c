...
void pwm_init(void){
	...
	TONE5.PULSE_WIDTH = (F_CPU/(32*D5));
	TONE6.PULSE_WIDTH = (F_CPU/(32*C5));
	TONE7.PULSE_WIDTH = (F_CPU/(32*B4));
...
void pwm_update(void){
	ATOMIC_BLOCK(ATOMIC_FORCEON){
		//TODO MODIFY ALL TCCRx TO NEW VALUES!
		/*TRY SETTING ONE OCRx SETTING TO '0', SPOOF MULTI IO*/
		if(!INPUT.SW0 && INPUT.SW1 && INPUT.SW2 && INPUT.SW3){
			//OCR0A = ((TONE0.PULSE_WIDTH*ADC_IN)%255);
			OCR0A = TONE0.PULSE_WIDTH;
			TCCR0B |= ((1<<CS01) | (1<<CS00)); 
		}
		else if(INPUT.SW0 && !INPUT.SW1 && INPUT.SW2 && INPUT.SW3){
			OCR1A = TONE1.PULSE_WIDTH;	
			...
			OCR1B = TONE3.PULSE_WIDTH;
			TCCR1B |= ((1<<CS21) | (1<<CS20));
		}
		else if(!INPUT.SW0 && !INPUT.SW1 && INPUT.SW2 && INPUT.SW3){
			OCR0B = (1<<TONE1.PULSE_WIDTH);	
			TCCR0B |= ((1<<CS01) | (1<<CS00)); 
		}
		else if(INPUT.SW0 && !INPUT.SW1 && !INPUT.SW2 && INPUT.SW3){
			OCR2A = (1<<TONE2.PULSE_WIDTH);	
			TCCR2B |= ((1<<CS21) | (1<<CS20));
		}
		else if(INPUT.SW0 && INPUT.SW1 && !INPUT.SW2 && !INPUT.SW3){
			OCR2B = (1<<TONE3.PULSE_WIDTH);
			TCCR2B |= ((1<<CS21) | (1<<CS20));
		}
		else{ 
			TCCR0B |= ((1<<CS01) | (1<<CS00));
...
ISR(PCINT1_vect){
	...
	INPUT.SW5 = (PINC & 0b00001000);
	INPUT.SW6 = (PINC & 0b00010000);
	INPUT.SW7 = (PINC & 0b00100000);
	update = true;	
