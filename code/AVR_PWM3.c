...
volatile uint8_t ADC2_IN;//TODO: USE THIS!?....
...
ISR(TIMER1_OVF_vect){
/*	ATOMIC_BLOCK(ATOMIC_FORCEON){
		while(!((ADC_RDY = ADCSRA) & 0b00010000));	
		ADC_IN = ADCH;
		ADMUX ^= (1<<MUX0);
...
int main (void)
{
	...
	//EICRA |= (1<<ISC00); /*EXTERNAL INTERRUPT CONTROL REGISTER A, ANY CHANGE*/	
	EICRA |= 0x00; /*EXTERNAL INTERRUPT CONTROL REGISTER A, LOW LEVEL INTERRUPT*/	
